...
public class FirebaseRepository<T> {
    ...
    public void query(Query query, OnFetchListListener<T> listener) {
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                    try {
                        ...
                        Log.e(TAG, "Data conversion error in query at key: " + dataSnapshot.getKey()
                                + " - Expected class: " + modelClass.getSimpleName() + ", Error: " + e.getMessage());
                    }
                }
                if (dataList.isEmpty()) {
            ...
            @Override
            public void onCancelled(@NonNull DatabaseError error) {
                listener.onFailure("Lỗi khi lấy dữ liệu truy vấn: " + error.getMessage());
            }
        });
    }
